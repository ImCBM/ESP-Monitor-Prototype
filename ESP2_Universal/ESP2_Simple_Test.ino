...
// Send callback (updated for new ESP32 core)
void onDataSent(const wifi_tx_info_t *tx_info, esp_now_send_status_t status) {
  char macStr[18];
...
// Send simple ping (ESP1 compatible envelope format)
void sendSimplePing() {
  messageCount++;
  
  // Create ESP1-compatible envelope JSON message
  String message = "{";
  message += "\"version\":\"5.0\",";
  message += "\"message_id\":\"ping_" + String(messageCount) + "_" + String(millis()) + "\",";
  message += "\"timestamp\":" + String(millis() / 1000) + ",";
  message += "\"shared_key\":\"ESP2_NETWORK_KEY\",";
  message += "\"message_type\":\"ping\",";
  
  // Source device info (ESP1 expects this structure)
  message += "\"source_device\":{";
  message += "\"device_id\":\"" + String(DEVICE_ID) + "\",";
  message += "\"owner\":\"test_user\",";
  message += "\"device_type\":\"ESP2_UNIVERSAL\",";
  ...
  message += "\"test_message\":\"Simple ping from ESP2 to ESP1 gateway\"";
  message += "}";
  message += "}";
  
  Serial.println("\n--- Sending ESP1 Compatible Ping ---");
